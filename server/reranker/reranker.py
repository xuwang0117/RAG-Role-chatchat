...
from configs import RE_SCORE_THRESHOLD
...
class LangchainReranker(BaseDocumentCompressor):
    ...
    def compress_documents(
        ...
        for value, index in zip(values, indices):
            ...
            if value > RE_SCORE_THRESHOLD or index < 3:
                final_results.append(doc)
        return final_results
    

    
    def compress_search_documents(
            self,
            documents: List,
            query: str,
            callbacks: Optional[Callbacks] = None,
        ...
        if len(documents) == 0:  # to avoid empty api call
            return []
        doc_list = documents
        sentence_pairs = [[query, _doc] for _doc in doc_list]
        results = self._model.predict(sentences=sentence_pairs,
                                      batch_size=self.batch_size,
                                      #  show_progress_bar=self.show_progress_bar,
                                      num_workers=self.num_workers,
